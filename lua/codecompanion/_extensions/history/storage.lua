...
function Storage:save_chat(chat)
    ...
    -- Validate and create snapshot atomically to minimize race condition window
    ...
    -- Create complete data snapshot immediately after validation
    log:trace("Saving chat: %s", chat.opts.save_id)
    local cwd = chat.opts.cwd or vim.fn.getcwd()
    local timestamp = os.time()

    -- Create chat data object with DEEP COPIES to prevent race conditions
    ---@type CodeCompanion.History.ChatData
    local chat_data = {
        save_id = chat.opts.save_id,
        title = chat.opts.title,
        messages = vim.deepcopy(chat.messages or {}), -- Deep copy!
        settings = vim.deepcopy(chat.settings or {}), -- Deep copy!
    ...
    -- Update index using the same processed data
